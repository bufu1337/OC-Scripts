...
function s.removeRSMonitor(monitor)
  s.m.send(s.rs.distributor, 478, s.mf.serial.serialize({"RSNet", method="push", storage=mod, rsmonitor=monitor, removing=true},true))
...
function s.RSMonitorON(mod, monitor)
  if s.rs.monitor[monitor] ~= nil and s.rs.monitor[monitor] ~= mod then
    if s.rs.monitor[monitor] ~= mod then
        s.m.send(s.rs.distributor, 478, s.mf.serial.serialize({"RSNet", method="push", storage=mod, rsmonitor=monitor},true))
        s.checkConnection(false)
    else
...
function s.RSMonitorOFF(monitor)
  if s.rs.monitor[monitor] ~= nil then
    if s.rs.monitor[monitor] ~= "" then
        s.m.send(s.rs.distributor, 478, s.mf.serial.serialize({"RSNet", method="pull", storage=s.rs.monitor[monitor], rsmonitor=monitor},true))
        s.checkConnection(false)
    else
...
function s.registerNetworkCards()
  s.m.send(s.rs.distributor, 478, s.mf.serial.serialize({"RSNet", "register"},true))
...
function s.checkConnection(askfor)
  ...
  if askfor == nil or askfor == true then
    s.m.send(s.rs.distributor, 478, s.mf.serial.serialize({"RSNet", "check"},true))
  end
  local _, localAddress, remoteAddress, port, distance, data = s.mf.event.pull(10, "modem_message")
  if data ~= nil then
    data = s.mf.serial.unserialize(data)
