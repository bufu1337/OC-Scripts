...
function RSC.Draw_GUI() 
  ...
  RSC.buttons.confirm.onTouch = function()
    ...
    if RSC.status == "register" then
      RSC.text.status.text = "Status: Registering Network-Cards"
      RSC.registerNetworkCards(RSC.combo.dis:getItem(RSC.itemselect.dis).text)
    elseif RSC.status == "remove" then
      ...
      RSC.removeRSMonitor(RSC.combo.dis:getItem(RSC.itemselect.dis).text, tonumber(RSC.inputs.remove.text))
    elseif RSC.status == "removeStation" then
      ...
      RSC.removeStation(RSC.combo.dis:getItem(RSC.itemselect.dis).text, RSC.inputs.dis.text)
    elseif RSC.status == "turnON" then
      ...
      RSC.RSMonitorON(RSC.combo.dis:getItem(RSC.itemselect.dis).text, RSC.text.rs.text:sub(19), tonumber(RSC.text.m2.text:sub(12,12)))
    elseif RSC.status == "turnOFF" then
      RSC.text.status.text = "Status: Turning OFF Monitor " .. RSC.text.m2.text:sub(12,12) .. "."
      RSC.RSMonitorOFF(RSC.combo.dis:getItem(RSC.itemselect.dis).text, tonumber(RSC.text.m2.text:sub(12,12)))
    end
  end
...
function RSC.RSMonitorON(distributor, station, mod, monitor)
  if RSC.rs.monitor[monitor] ~= nil and RSC.rs.monitor[monitor] ~= mod then
    if RSC.rs.monitor[monitor] ~= mod then
        ...
        RSC.checkConnection(RSC.combo.dis:getItem(RSC.itemselect.dis).text, false)
    else
...
function RSC.removeRSMonitor(distributor, station, monitor)
  RSC.ocnet.send(RSC.mf.serial.serialize({"RSNet", method="push", storage="", rsmonitor=monitor, removing=true},true))
  RSC.checkConnection(RSC.combo.dis:getItem(RSC.itemselect.dis).text, false)
end
