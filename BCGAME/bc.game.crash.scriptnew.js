var config = {
  ...
  basePayout: {label: 'Starting Payout:', value: 10, type: 'number'},
  maxPayout: {label: 'Maximum Payout:', value: 10000, type: 'number'},
  minPayout: {label: 'Minimum Payout:', value: 10, type: 'number'},
  ...
  onBetLossTitle: {label: 'Loss Bet Settings', type: 'title'},
    onBetLoss: {label: '', value: '0', type: 'radio',
      options: [
  ...
  onPayoutLossTitle :{label: 'Loss Payout Settings', type: 'title'},
    onPayoutLoss: {label: '', value: '1', type: 'radio',
      options: [
  ...
  onPayoutWinTitle :{label: 'Win Payout Settings', type: 'title'},
    onPayoutWin: {label: '', value: '0', type: 'radio',
      options: [
        {value: '0', label: 'Return to Starting Payout'},
        {value: '1', label: 'Add to Payout'},
        {value: '2', label: 'Subtract From Payout'},
...
function main (){
  ...
  var gameStatus = 1
  var gameCounter = 1
  var lossReset = config.lossReset.value;
  var lossCounter = 0
        ...
        {if (payout > 1)
            ...
            {if(gameStatus)
                {gameStatus--}
                    ...
                    lossCounter++
                    log.error('We lost that one. Loss streak: '+ lossCounter)
...
function gameStarter(calculatedBet, calculatedPayout)
    {if (lossCounter >= lossReset)
        {calculatedBet = currentBet * 2
            calculatedPayout = currentPayout / 2 + 1.02
                log.error('Loss Streak reached! Bet: ' + calculatedBet + ' Payout: ' + calculatedPayout);
                lossCounter = 0
    }
    else
...
function onPayoutLossSwitch (switchValue){
    ...
    switch(switchValue){
        ...
        case '1':{
            calPayout = currentPayout + addPayoutLoss
            break;
        }
        case '2':{
            calPayout = currentPayout - subPayoutLoss
            break;
        }
        case '3':{
            calPayout = currentPayout * mulPayoutLoss
            break;
        }
        case '4':{
            calPayout = currentPayout / divPayoutLoss
            break;
        }
    }
    switch(switchValue){
